...
def deskew(image):
    ...
    deskewed_image = cv2.warpAffine(image, M, (w, h), flags=cv2.WARP_INVERSE_MAP | cv2.INTER_LINEAR)
    return deskewed_image


def cleanup_and_standardize_char(char_image_bgr):
    """
    Hàm chính nhận một ảnh ký tự ĐÃ ĐƯỢC CẮT (dạng BGR) và thực hiện pipeline làm sạch.
    Trả về ảnh 28x28 cuối cùng để đưa vào model.
    """
    ...
    # --- BƯỚC 1 CẢI TIẾN: Chỉnh thẳng ký tự (Deskewing) ---
    ...
    # --- BƯỚC 2 CẢI TIẾN: Căn giữa theo khối lượng (Center of Mass) ---
